...
`throughput`, `time`, `ppo`, `policy`, `env`, `ladder`, `health`. A run with a config section
from an installed package can add that package's own group after them; RoyaleImitate's `imitation`
is one, and its docs list its keys. On an iteration where a run holds its bot still
(`ppo/actor_frozen` 1), the `ppo/` keys computed from the bot's own update, such as
