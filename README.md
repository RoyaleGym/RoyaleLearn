...
| [RoyaleImitate](https://github.com/RoyaleGym/RoyaleImitate) | an optional add-on: config sections that start a bot from saved weights and keep it near a reference policy while it learns | a package this repo finds through its extension API ([`docs/extensions.md`](docs/extensions.md)) when a config names one of its sections. A run that names none never imports it |
